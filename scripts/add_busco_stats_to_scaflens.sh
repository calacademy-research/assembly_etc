...
# 11Jun2025 remove older busco info if it is there before adding any new stats
...
function add_busco_info {
   ...
   ' <(busco_scaffstats.sh <(get_scafforder))  <(get_scaflens)
}

function append_summary {  # echo the summary of the full BUSCO run at the bottom of the new scaflens file
   summary_from_busco_full_tables.sh $scaff_order |
   awk '
...
function msg { >&2 echo -e "$@"; } # write a msg to stderr
...
function get_scaflens {
   remove_busco_anno_from_scaflens.sh $scaflens
}
